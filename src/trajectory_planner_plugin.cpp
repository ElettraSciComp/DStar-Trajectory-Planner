...
void DStarGlobalPlanner::configure(
    ...
    if(!initialized)
    {
        nav2_util::declare_parameter_if_not_declared(node_, "goal_distance_threshold", rclcpp::PARAMETER_DOUBLE); 
        goal_distance_threshold = node_->get_parameter("goal_distance_threshold").as_double();
        nav2_util::declare_parameter_if_not_declared(node_, "neighbor_distance_threshold", rclcpp::PARAMETER_DOUBLE);
        neighbor_distance_threshold = node_->get_parameter("neighbor_distance_threshold").as_double();
        nav2_util::declare_parameter_if_not_declared(node_, "occupancy_threshold", rclcpp::PARAMETER_INTEGER);
        occupancy_threshold = node_->get_parameter("occupancy_threshold").as_int();
        nav2_util::declare_parameter_if_not_declared(node_, "cutoff_distance", rclcpp::ParameterValue(30));
        cutoff_distance = node_->get_parameter("cutoff_distance").as_int();
        nav2_util::declare_parameter_if_not_declared(node_, "trajectory_optimizer.repulsion_gain", rclcpp::PARAMETER_DOUBLE);
        repulsion_gain = node_->get_parameter("trajectory_optimizer.repulsion_gain").as_double();
        nav2_util::declare_parameter_if_not_declared(node_, "trajectory_optimizer.potential_field_radius", rclcpp::PARAMETER_DOUBLE);
        potential_field_radius = node_->get_parameter("trajectory_optimizer.potential_field_radius").as_double();
        nav2_util::declare_parameter_if_not_declared(node_, "erosion.enable", rclcpp::PARAMETER_BOOL);
        erosion = node_->get_parameter("erosion.enable").as_bool();
        nav2_util::declare_parameter_if_not_declared(node_, "erosion.erosion_gap", rclcpp::PARAMETER_INTEGER);
        erosion_gap = node_->get_parameter("erosion.erosion_gap").as_int();
...
nav_msgs::msg::Path DStarGlobalPlanner::createPlan(
    ...
    //nav_msgs::msg::Path an array of posed stamped
